...
TEMPERATURE = 0.5
ESSAIS_MAX = 2
FICHIER_VOIES = os.path.join("data", "voies.json"); DELAI_DISPONIBILITE = 5; DELAI_GENERATION = 120
...
def disponible():
    try:
        import ollama
        ollama.Client(timeout=DELAI_DISPONIBILITE).list()
        return True
    except Exception:
        return False
...
def brouillon(c, voies=None):
    ...
    if detail:
        if len(detail) >= 3:
            para.append(detail[0] + ", " + detail[1] + ". " + detail[2] + ".")
        else:
...
def defauts(texte, source):
    ...
    # Une evolution ne peut etre citee que si le brouillon en parle.
    if "puis" not in source:
        for mot in ("augment", "entre les deux trimestres", "s'est amelior",
                    "s'est amélior", "a progress", "a evolu", "a évolu"):
            if mot in bas:
                problemes.append(f"invente une evolution : {mot}")

    # Si le brouillon dit que les notes baissent, le texte ne peut pas
    # dire l'inverse.
    if "en baisse" in source or "ont baisse" in source:
        for mot in ("amelior", "amélior", "progress", "en hausse", "monte"):
            if mot in bas:
                problemes.append(f"contredit la baisse : {mot}")
    ...
    if "Concretement" in source and "professeur principal" in source:
